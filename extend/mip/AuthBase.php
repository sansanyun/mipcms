...
//      }
        if ($passAuthInfo) {
            ...
            $isCheck = false;
            $rolesNodeAllList = db('RolesNode')->select();
            if ($rolesNodeAllList) {
                foreach ($rolesNodeAllList as $k => $v) {
                    if (strtoupper($this->request->action()) == $v['name']) {
                        $isCheck = true;
                    }
                }
            }
