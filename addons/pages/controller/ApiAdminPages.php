...
use mip\AdminBase;
class ApiAdminPages extends AdminBase
{
    ...
    public function itemAdd()
    {
        ...
        $itemInfo = db('pages')->where('title',$title)->find();
        if ($itemInfo) {
          return jsonError('名称已存在，请重新输入');
        }
        $itemInfo = db('pages')->where('url_name',$url_name)->find();
        if ($itemInfo) {
          return jsonError('别名已存在，请重新输入');
        }
        db('pages')->insert(array(
                'id' => uuid(),
                'title' => $title,
                'url_name' => $url_name,
                'template' => $template,
    ...
    public function itemEdit()
    {
        ...
        $itemInfo = db('pages')->where('id',$id)->find();
        if (!$itemInfo) {
          return jsonError('修改项不存在');
        }
        $itemInfo = db('pages')->where('id','<>',$id)->where('title',$title)->find();
        if ($itemInfo) {
          return jsonError('名称已存在，请重新输入');
        }
        $itemInfo = db('pages')->where('id','<>',$id)->where('url_name',$url_name)->find();
        if ($itemInfo) {
          return jsonError('别名已存在，请重新输入');
        }
        
        db('pages')->where('id',$id)->update(array(
                'title' => $title,
                'url_name' => $url_name,
                'keywords' => $keywords,
                'template' => $template,
