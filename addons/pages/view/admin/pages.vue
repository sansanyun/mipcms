...
<div>
    <header class="mipcms-container-header clearfix">
        <div class="header-group">
            <h4 class="title">页面</h4> <h5 class="sub-title">单页面</h5>
        </div>
                ...
                <div v-if='itemList.length' class="diy-table-body">
                    <div class="diy-table-item" v-for='item in itemList' >
                         <ul class="list-unstyled row">
                            ...
                            <li class="col-md-3">
                                <span>{$webUrl}/{{item.url_name}}.html</span>
                            </li>
        ...
        methods: {
            ...
            itemDel(item) {
                ...
                this.$Modal.confirm({
                    ...
                    onOk: () => {
                        _this = this;
                        this.$mip.ajax('{$domain}/{$Think.config.admin}/ApiAdminPages/itemDel', {
                            id: item.id,
                        }).then(function(res) {
                            if(res.code == 1) {
            ...
            itemPost(val) {
                ...
                this.$refs[val].validate((valid) => {
                    if(valid) {
                        ...
                        if (this.item.id) {
                            this.$mip.ajax('{$domain}/{$Think.config.admin}/ApiAdminPages/itemEdit', {
                                id: this.item.id,
                            ...
                            this.$mip.ajax('{$domain}/{$Think.config.admin}/ApiAdminPages/itemAdd', {
                                title: this.item.title,
                                url_name: this.item.url_name,
                                template: this.item.template,
                                content: this.content,
            ...
            getItemList() {
                this.loading = true;
                var _this = this;
                this.$mip.ajax('{$domain}/{$Think.config.admin}/ApiAdminPages/itemSelect', {
