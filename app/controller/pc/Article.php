...
class Article extends Mip
{
    ...
    public function index()
    {
        ...
        if ($category) {
            ...
            if ($this->mipInfo['superSites']) {
                $this->assign('crumb',false);
                $this->assign('crumbCategoryName',$categoryInfo['name']);
                $this->assign('crumbCategoryUrl',$this->mipInfo['httpType'] . $categoryInfo['url_name'] . '.' . $this->mipInfo['topDomain'] .'/');
            } else {
            ...
            if ($sub) {
                ...
                if ($this->mipInfo['superSites']) {
                    ...
                    if ($this->mipInfo['aritcleLevelRemove']) {
                        $this->assign('crumbCategorySub',true);
                        $this->assign('crumbCategorySubName',$subInfo['name']);
                        $this->assign('crumbCategorySubUrl', $this->articleDomain . $this->rewrite . '/' . $categoryInfo['url_name'] . '/' . $subInfo['url_name'] . '/');
                        //标题关键词描述
                        $this->assign('mipTitle', $subInfo['seo_title'] ? $subInfo['seo_title'] : $subInfo['name'] . $this->mipInfo['titleSeparator'] . $categoryInfo['name']);
    ...
    public function articleDetail() {
        ...
        if ($itemInfo['categoryInfo']) {
            ...
            if ($this->mipInfo['superSites']) {
                ...
                if ($itemInfo['categoryInfo']['pid'] != 0) {
                    $subInfo = $itemInfo['categoryInfo'];
                    $categoryInfo = ArticlesCategory::get($subInfo['pid']);
                    //当前所属分类别名
                    ...
                    $this->assign('crumbCategorySub',true);
                    $this->assign('crumbCategorySubName',$subInfo['name']);
                    $this->assign('crumbCategorySubUrl',$this->mipInfo['httpType'] . $categoryInfo['url_name'] . '.' . $this->mipInfo['topDomain'] . $this->rewrite . '/' . $subInfo['url_name'] . '/');
                    //标题关键词描述
                    $this->assign('mipTitle', $itemInfo['title'] . $this->mipInfo['titleSeparator'] . $categoryInfo['name']);
                    $itemInfo['categoryInfo'] = $subInfo;
                } else {
                    $this->assign('crumbCategoryName',$itemInfo['categoryInfo']['name']);
                    $this->assign('crumbCategoryUrl',$this->mipInfo['httpType'] . $itemInfo['categoryInfo']['url_name'] . '.' . $this->mipInfo['topDomain'] . $this->rewrite . '/');
                    $this->assign('crumbCategorySub',false);
                    //标题关键词描述
                    $this->assign('mipTitle', $itemInfo['title'] . $this->mipInfo['titleSeparator'] .$itemInfo['categoryInfo']['name']);
                }

            } else {
                $this->assign('crumbCategoryName',$itemInfo['categoryInfo']['name']);
                if ($this->mipInfo['aritcleLevelRemove']) {
                    $this->assign('crumbCategoryUrl',$this->articleDomain . '/' . $itemInfo['categoryInfo']['url_name'] . '/');
                } else {
                    $this->assign('crumbCategoryUrl',$this->articleDomain . '/' . $this->articleModelUrl .'/' . $itemInfo['categoryInfo']['url_name'] . '/');
                }
                if ($itemInfo['categoryInfo']['pid'] != 0) {
                    ...
                    if ($this->mipInfo['aritcleLevelRemove']) {
                        ...
                        $this->assign('crumbCategorySub',true);
                        $this->assign('crumbCategorySubName',$subInfo['name']);
                        $this->assign('crumbCategorySubUrl',$this->articleDomain . '/' . $itemInfo['categoryInfo']['url_name'] . '/' . $subInfo['url_name'] . '/');
                        $itemInfo['categoryInfo'] = $subInfo;
                    }
                }
                //标题关键词描述
        ...
        if ($this->mipInfo['articlePages']) {
            ...
            if ($this->mipInfo['diyUrlStatus']) {
                if ($diy) {
                $itemInfo['pageCode'] = $CP->pagenav($currentPageNum,$this->articleDomain.'/'.$this->articleModelUrl . '/' . $diy);
                }
            } else {
                $itemInfo['pageCode'] = $CP->pagenav($currentPageNum,$this->articleDomain.'/'.$this->articleModelUrl . '/' . $id);
            }
        }
        //标签关键词
