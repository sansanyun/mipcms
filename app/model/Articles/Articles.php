...
class Articles extends ModelBase
{
    public function getItemList($cid, $page = 1, $perPage = 10, $orderBy = 'publish_time', $order = 'desc', $where = null)
    {
        if ($cid == 0) {
            ...
            if ($articlesCategoryList) {
                $itemList = $this->where($where)->whereOr('cid',$cid)->whereOr('cid','in',$cids)->page($page,$perPage)->order($orderBy,$order)->select();
            } else {
        ...
        if ($itemList) {
            foreach($itemList as $k => $v) {
                ...
                $itemList[$k]['articlesLastCategory'] = null;
                if ($itemList[$k]['articlesCategory']) {
                    $itemList[$k]['articlesLastCategory'] = articlesCategory::where('id',$itemList[$k]['articlesCategory']['pid'])->find();
                }
    ...
    public function domainUrl($item)
    {
        ...
        if ($this->mipInfo['urlCategory']) {
            $item['articlesCategory'] = articlesCategory::where('id',$item['cid'])->find();
            if ($item['articlesCategory']) {
                $item['articlesLastCategory'] = articlesCategory::where('id',$item['articlesCategory']['pid'])->find();
                if ($item['articlesLastCategory']) {
                    if ($this->mipInfo['domain']) {
                        $res = $this->mipInfo['httpType'] . $this->mipInfo['domain'] . $this->rewrite . '/' . $item['articlesLastCategory']['url_name'] . '/' . $item['articlesCategory']['url_name'] . '/' . $tempId . '.html';
                    } else {
                        $res = $this->rewrite . '/' . $item['articlesLastCategory']['url_name'] . '/' . $item['articlesCategory']['url_name'] . '/' . $tempId . '.html';
                    }
                } else {
                    if ($this->mipInfo['domain']) {
                        $res = $this->mipInfo['httpType'] . $this->mipInfo['domain'] . $this->rewrite . '/' . $item['articlesCategory']['url_name'] . '/' . $tempId . '.html';
                    } else {
                        $res = $this->rewrite . '/' . $item['articlesCategory']['url_name'] . '/' . $tempId . '.html';
                    }
                }
            }
        }
    ...
    public function domainMipUrl($item)
    {
        ...
        if ($this->mipInfo['urlCategory']) {
            ...
            if ($item['articlesCategory']) {
                $item['articlesLastCategory'] = articlesCategory::where('id',$item['articlesCategory']['pid'])->find();
                if ($item['articlesLastCategory']) {
                    $res = $this->mipInfo['httpType'] . $this->mipInfo['mipDomain'] . $this->rewrite . '/' . $item['articlesLastCategory']['url_name'] . '/' . $item['articlesCategory']['url_name'] . '/' . $tempId . '.html';
                } else {
                    $res = $this->mipInfo['httpType'] . $this->mipInfo['mipDomain'] . $this->rewrite . '/' . $item['articlesCategory']['url_name'] . '/' . $tempId . '.html';
                }
            }
